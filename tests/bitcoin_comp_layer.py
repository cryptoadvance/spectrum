...
class SpecterAuthServiceProxy(AuthServiceProxy):
    ...
    def __init__(self, auth_service_proxy):
        ...
        self.ensure_ascii = auth_service_proxy.ensure_ascii  # can be toggled on the fly by tests
...
class SpecterBitcoinTestFramework(BitcoinTestFramework, metaclass=SpecterBitcoinTestMetaClass):
    ...
    def bitcoin_rpc(self, node_number=0):
        ...
        rpc = BitcoinRPC(host="127.0.0.1", user="__cookie__", password=password.strip(), port=rpc_port(node_number))
        return rpc
    ...
    def main(self):
        ...
        try:
            ...
            if exit_code != 0:
                raise Exception(f"bitcoind did not shutdown cleanly but with exit_code {exit_code}")
